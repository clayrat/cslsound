...
From cslsound Require Import Basic Lang CSLsound.
...
Inductive rgsep_assn :=
  ...
  | RGex [A] (pp: A -> rgsep_assn).

(** Separating conjunction of a finite seq of assertions is just a
  derived assertion. *)

Fixpoint RGistar ps :=
  if ps is p::s
    then RGstar p (RGistar s)
    else RGlocal Aemp.
...
Fixpoint RGsat ss p :=
  match p with
    | RGlocal p      => sat ss.1 p
    | RGshared r p   => (forall x, ss.1.2 x = None) /\ sat (ss.1.1, ss.2 r) p
    | RGconj p q     => RGsat ss p /\ RGsat ss q
    | RGdisj p q     => RGsat ss p \/ RGsat ss q
    | RGstar p q     => exists h1 h2, RGsat (ss.1.1, h1, ss.2) p
                          /\ RGsat (ss.1.1, h2, ss.2) q
                          /\ hdef h1 h2 /\ hplus h1 h2 = ss.1.2
    | RGex _ pp      => exists v, RGsat ss (pp v)
  end.
...
Lemma sat_istar_map_expand :
  forall r l ss (f : int -> rgsep_assn) (IN: r \in l) (DL: uniq l),
    RGsat ss (RGistar (map f l))
    <-> exists h1 h2, RGsat (ss.1.1, h1, ss.2) (f r)
      /\ RGsat (ss.1.1, h2, ss.2) (RGistar (map f (remove r l)))
      /\ hdef h1 h2 /\ hplus h1 h2 = ss.1.2.
Proof.
move=>r l [[s h] ss] f.
elim: l=>//= a l H in h *; rewrite inE=>/orP H1 /andP [Ha Hu]; case: H1.
...
Fixpoint hplus_list l :=
  if l is h :: t
    then hplus h (hplus_list t)
    else fun => None.

Definition RGdef (l: seq rname) (hh : rname -> heap) :=
...
(* Condition (v) *)
       /\ (forall hJ hF c' ss'
             ...
             (EQ: hJ = hplus_list (map hh (lminus (locked c') (locked c))))
             (D1: hdef h hF)
             (RD: RGdef (lminus (locked c') (locked c)) hh)
             (Dh: forall r (IN: In r (locked c')) (NIN: ~ In r (locked c)),
                  hdef (hplus h hF) (hh r)),
             exists h' hh',
               snd ss' =
                 hplus h'
                   (hplus (hplus_list (map hh' (lminus (locked c) (locked c')))) hF)
               /\ hdef h' hF
               /\ RGdef (lminus (locked c) (locked c')) hh'
               /\ forall r (IN: In r (locked c)) (NIN: ~ In r (locked c')),
                           hdef (hplus h' hF) (hh' r)
               /\ forall r (IN: In r (locked c)) (NIN: ~ In r (locked c')),
                            guar r (hh r) (hh' r)
               /\ forall r (OTH: ~ In r (locked c) \/ In r (locked c')),
                            hh r = hh' r
...
Fixpoint fvAA p :=
  match p with
    | RGlocal P  | RGshared _ P            => fvA P
    | RGconj P Q | RGstar P Q | RGdisj P Q => fun v => fvAA P v \/ fvAA Q v
    | RGex _ P                             => fun v => exists x, fvAA (P x) v
  end.
...
Fixpoint frgnA p :=
  match p with
    ...
    | RGstar P Q | RGconj P Q | RGdisj P Q => fun v => frgnA P v \/ frgnA Q v
    | RGex _ P                             => fun v => exists x, frgnA (P x) v
  end.
...
Proof.
move=>p s s' A h hh; elim: p h A=>/=.
- by move=>p h A; apply: prop1_A.
- by move=>r p h A; apply: and_iff_compat_l; apply: prop1_A.
- move=>p IH1 q IH2 h A.
  rewrite IH1; last by move=>v H; apply: A; left.
  by rewrite IH2 // =>v H; apply: A; right.
- move=>p IH1 q IH2 h A.
  rewrite IH1; last by move=>v H; apply: A; left.
  by rewrite IH2 // =>v H; apply: A; right.
- move=>p IH1 q IH2 h A.
  apply: ex_iff=>h1; apply: ex_iff=>h2.
  rewrite IH1; last by move=>v H; apply: A; left.
  by rewrite IH2 // =>v H; apply: A; right.
move=>T pp IH h A; apply: ex_iff=>x.
by apply: IH=>v H; apply: A; exists x.
Qed.
...
Lemma hdef_hplus_list A h hh (l: seq A) :
...
{ (* Step *)
  ...
  { (* C1 does a step *)
    ...
    assert (LL : forall r, In r (lminus (locked c1') (locked C1)) ->
                 hdef h2 (hh r)).
    ...
    exploit (ROK2 hh'); ins; rewrite ?In_app in *; desf; eauto.
       intros; destruct (In_dec Z.eq_dec r (lminus (locked C1) (locked c1'))).
  ...
  { (* C2 does a step *)
    ...
    assert (LL : forall r, In r (lminus (locked c2') (locked C2)) ->
                 hdef h1 (hh r)).
      by intros; rewrite In_list_minus in *;
         specialize (Dh r); rewrite !in_app_iff, !hdef_hplus in Dh;
         apply Dh; ins; desf; eauto.
    ...
    exploit (LOK2 hh'); ins; rewrite ?In_app in *; desf; eauto.
       intros; destruct (In_dec Z.eq_dec r (lminus (locked C2) (locked c2'))).
         by intuition; rewrite In_list_minus in *; desf; eauto.
...
Proof.
  ...
  (* Step *)
  assert (LL : forall r, In r (lminus (locked c') (locked C)) -> hdef hR (hh r)).
    by intros; rewrite In_list_minus in *;
  ...
  assert (LL : forall r, In r (lminus (locked C) (locked c')) ->
    hdef hR (hh' r)).
    intros; rewrite In_list_minus in *.
    by specialize (Dh' r); rewrite !hdef_hplus in Dh'; tauto.
...
Proof.
  ...
  generalize (lminus l l'); clear l l'; intro l.
